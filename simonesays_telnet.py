...
if tn:
	print "Telnet connected to server"
else:
	print "WARNING: Could not connect to telnet server"
...
try:
	...
	inFile = codecs.open("simonescript.txt", "r", "utf-8")
except OSError:
	#File not found
...
def talk():
	...
	if(len(speechCandidates)>0):
		...
		say_sentence_to_say = 'say ' + sentence_to_say
#		system(voice_command + sentence_to_say.encode("mac-roman"))
		# subprocess.call(['say', '-v', voice, sentence_to_say])
		if tn: # if telnet server port is open!
			say_sentence_to_say = (say_sentence_to_say.encode('utf-8'))
			tn.write(say_sentence_to_say+'\n')
			#tn.write('say hej'+'\n')
			print ("telnet: " + say_sentence_to_say)
		else: # otherwise just use internal MacOS speech
			subprocess.call(['say', '-v', voice, sentence_to_say])
...
key_listener_thread = threading.Thread(target = keyListener, name="keylistener", args=("keylistener",))
